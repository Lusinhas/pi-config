    ...
    switch (block.type) {
      ...
      case "resource": {
        ...
        if (typeof resource.text === "string") {
          ...
          if (size <= inlineLimit) parts.push(`[resource ${uri}]\n${resource.text}`);
          else parts.push(`[resource ${uri}: ${formatBytes(size)} of text, too large to inline]`);
...
export class McpClient {
  ...
  async listTools(): Promise<McpToolDef[]> {
    ...
    const raw = await this.paginate("tools/list", "tools");
  ...
  async listPrompts(): Promise<McpPromptDef[]> {
    ...
    const raw = await this.paginate("prompts/list", "prompts");
  ...
  async listResources(): Promise<McpResourceDef[]> {
    ...
    const raw = await this.paginate("resources/list", "resources");
  ...
  async callTool(name: string, args: Record<string, unknown>, options?: RequestOptions): Promise<McpToolCallResult> {
    ...
    if (Array.isArray(record.content)) {
      for (const block of record.content) {
        if (isRecord(block) && typeof block.type === "string") content.push(block as McpContentBlock);
      }
    }
    ...
    if (Array.isArray(record.messages)) {
      for (const message of record.messages) {
        ...
        for (const block of candidates) {
          if (isRecord(block) && typeof block.type === "string") blocks.push(block as McpContentBlock);
        }
  ...
  private async paginate(method: string, key: string): Promise<Record<string, unknown>[]> {
    ...
    for (let page = 0; page < MAX_PAGES; page += 1) {
      ...
      try {
        ...
        if (out.length === 0 && isMethodNotFound(error)) return [];
        throw error;
      }
      const record = isRecord(result) ? result : {};
  ...
  private async request(
    ...
    if (options?.signal?.aborted) throw new Error(`MCP request "${method}" was aborted`);
