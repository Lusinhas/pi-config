...
import type { CapReason, ModelSource, Runner } from "./runner.ts"
...
export function parseTeam(raw: unknown): TeamDefinition | string {
  if (Array.isArray(raw)) {
    ...
    if (members.length === 0) return "team has no members"
  ...
  if (typeof raw === "object" && raw !== null) {
    ...
    if (members.length === 0) return "team has no members (expected a non-empty \"members\" array of agent names)"
...
export async function runTeam(runner: Runner, registry: AgentRegistry, name: string, raw: unknown, task: string, context: string | undefined, source: ModelSource & { cwd: string }, signal: AbortSignal | undefined, progress?: (text: string) => void): Promise<TeamResult> {
  const team = parseTeam(raw)
  if (typeof team === "string") throw new Error(`subagents: team "${name}" is invalid: ${team}`)
    ...
    pending.push(
      runner
        .withSlot(async () => {
          ...
          emit()
          const outcome = await runner.runAgent(definition, memberTask, context, source, signal, (turns) => {
  ...
  const sections = reports.map((report) => {
    let heading: string
    if (report.status === "completed") heading = `completed in ${report.turns} turn${report.turns === 1 ? "" : "s"}`
    else if (report.status === "capped") heading = `stopped at the ${report.capped} cap after ${report.turns} turns`
    else heading = "failed"
    return `## ${report.agent} — ${heading}\n\n${report.detail}`
  })
  const text = `# Team ${name} report (${completed}/${reports.length} members completed)\n\n${sections.join("\n\n")}`
  return {
    text,
    details: {
      team: name,
