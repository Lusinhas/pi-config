...
export function buildTranscript(entries: unknown[], maxChars: number): string {
  ...
  for (const entry of entries) {
    ...
    if (isRecord(entry.message)) message = entry.message
    else if (typeof entry.role === "string") message = entry
    ...
    else if (role === "toolResult" || role === "tool") label = "Tool"
    else label = role
...
function readEntries(sessionManager: unknown): unknown[] {
  if (!isRecord(sessionManager)) return []
...
export async function runAdvisor(question: string, config: SubagentsConfig, runner: Runner, ctx: AdvisorContext, signal: AbortSignal | undefined, onTurn?: (turns: number) => void): Promise<AdvisorResult> {
  const trimmed = question.trim()
  if (trimmed === "") throw new Error("advisor: a question is required")
  ...
  const resolved = await resolveModel(config.advisorModel, ctx, ctx.cwd)
  ...
  if (result.text.trim() === "") throw new Error("advisor: the advisor model returned no advice")
  return {
    text: result.text,
    details: {
      model: resolved.id,
