...
function buildSignal(timeoutMs: number, outer: AbortSignal | undefined): AbortSignal | undefined {
...
export async function judgeAction(
  ...
  try {
    ...
    const timeoutMs = Math.max(1000, config.timeoutMs);
    ...
    if (origin !== "") parts.push(`Origin: subagent "${origin}"`);
    ...
    const response = await completeSimple(
      ...
      },
      {
        ...
        signal: buildSignal(timeoutMs, ctx.signal),
        timeoutMs,
