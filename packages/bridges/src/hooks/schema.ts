...
export function emptyEvents(): Record<HookEventName, HookGroup[]> {
  return {
    PreToolUse: [],
    PostToolUse: [],
    UserPromptSubmit: [],
...
export function emptyLoadedHooks(): LoadedHooks {
  return { events: emptyEvents(), problems: [], sources: [], totalHooks: 0 }
}

export class HookLoader {
  paths(cwd: string): string[] {
    return [join(homedir(), ".pi", "agent", "hooks.json"), join(cwd, ".pi", "hooks.json")]
  }

  load(cwd: string, defaultTimeoutMs: number): LoadedHooks {
    const loaded: LoadedHooks = emptyLoadedHooks()

    for (const path of this.paths(cwd)) {
      let raw: string
      ...
      this.mergeFile(loaded, parsed, path, defaultTimeoutMs)
    }

    return loaded
  }

  mergeFile(loaded: LoadedHooks, parsed: unknown, path: string, defaultTimeoutMs: number): void {
    if (!isRecord(parsed)) {
    ...
    for (const [eventName, value] of Object.entries(map)) {
      ...
      value.forEach((entry, index) => {
        ...
        const group = this.parseGroup(entry, label, path, defaultTimeoutMs, loaded.problems)
