...
export function coerceJsonRpc(value: unknown): JsonRpcMessage | null {
  ...
  if (typeof value.method === "string") {
    ...
    if (hasId && id !== null) {
      const request: JsonRpcRequest = { jsonrpc: "2.0", id: id as number | string, method: value.method };
  ...
  if (hasId && ("result" in value || "error" in value)) {
    ...
    return { jsonrpc: "2.0", id: id as number | string | null, result: value.result };
  }
...
const TERM_DELAY_MS = 2000;
const CLOSE_GIVEUP_MS = 4000;
...
export class StdioTransport implements Transport {
  ...
  constructor(options: StdioTransportOptions) {
    ...
    this.stderrTail = new StderrTail(options.stderrLines);
  }
  ...
  async close(): Promise<void> {
    ...
    await new Promise<void>((resolve) => {
      ...
      const giveUp = setTimeout(() => resolve(), CLOSE_GIVEUP_MS);

      if (typeof giveUp.unref === "function") {
        giveUp.unref();
      }
...
export class HttpTransport implements Transport {
    ...
    try {
      ...
      if (contentType.includes("text/event-stream")) {
        if (response.body !== null) {
          await this.readSse(response.body, controller.signal);
        }
  ...
  async close(): Promise<void> {
    ...
    try {
      ...
      await fetch(this.options.url, { method: "DELETE", headers, signal: AbortSignal.timeout(DELETE_TIMEOUT_MS) });
    } catch {
      return;
    }
  }
