...
export async function discoverEndpoints(serverUrl: string, wwwAuthenticate: string | null): Promise<OauthEndpoints> {
  ...
  if (headerMatch !== null) {
    resourceMeta = await fetchJson(headerMatch[1]);
  }
  ...
  if (resourceMeta !== null) {
    ...
    if (typeof resourceMeta.resource === "string" && resourceMeta.resource !== "") {
...
export function buildStoredToken(
...
export async function registerClient(
  ...
  try {
    ...
    throw new Error(`dynamic client registration failed: ${describeError(error)}`);
  }
...
export function startLoopback(): Promise<Loopback | null> {
  return new Promise((resolve) => {
    ...
    const server = createServer((request, response) => {
      ...
      if (code !== null && code !== "") {
        response.end(
          "<html><body><p>Authorization received. You can close this tab and return to pi.</p></body></html>",
        );
        settle({ code, state: url.searchParams.get("state") });
      } else {
        const errorParam = url.searchParams.get("error") ?? "unknown error";
        response.end(
          `<html><body><p>Authorization failed: ${errorParam}. Return to pi and try again.</p></body></html>`,
        );
        settle(null);
      }
    });
...
export function parsePasted(text: string): AuthCode | null {
...
export function waitForGrant(
  ...
  return new Promise((resolve) => {
    ...
    if (loopback !== null) {
      void loopback.wait.then(accept, () => accept(null));
    }
...
export class TokenStore {
  ...
  constructor(path: string = join(homedir(), ".pi", "agent", "suite.json")) {
    this.path = path;
  }

  load(name: string): StoredToken | null {
    const raw = this.tokensOf(this.readRoot())[name];

    if (!isRecord(raw) || typeof raw.accessToken !== "string" || raw.accessToken === "") {
      return null;
    }
  ...
  save(name: string, token: StoredToken | null): void {
    try {
  ...
  tokensOf(root: Record<string, unknown>): Record<string, unknown> {
    const mcp = isRecord(root.mcp) ? root.mcp : {};
...
export class OAuth {
  ...
  constructor(tokens: TokenStore = new TokenStore()) {
    this.tokens = tokens;
  }

  loadToken(name: string): StoredToken | null {
    return this.tokens.load(name);
  }

  async getAccessToken(name: string): Promise<string | null> {
    const token = this.tokens.load(name);
    ...
    if (token.expiresAt !== null && Date.now() >= token.expiresAt) {
      const refreshed = await this.refreshStoredToken(name, token);
  ...
  private async refreshStoredToken(name: string, token: StoredToken): Promise<StoredToken | null> {
    if (token.refreshToken === null || token.tokenEndpoint === "" || token.clientId === "") {
      return null;
    }
    ...
    if (token.resource !== "") {
      fields.resource = token.resource;
    }

    if (token.scope !== null) {
      fields.scope = token.scope;
    }
    ...
    try {
      ...
      this.tokens.save(name, refreshed);
      return refreshed;
    } catch {
      this.tokens.save(name, null);
      return null;
    }
  }
  ...
  async authorize(
    ...
    if (!ctx.hasUI) {
      throw new Error("MCP OAuth authorization requires an interactive session");
    }
    ...
    if (endpoints.registrationEndpoint !== null) {
      ...
      const stored = this.tokens.load(name);
    ...
    if (endpoints.scope !== null) {
      authUrl.searchParams.set("scope", endpoints.scope);
    }
    ...
    if (granted === null) {
      throw new Error("authorization was not completed before the timeout");
    }

    if (granted.state !== null && granted.state !== state) {
      throw new Error("authorization state mismatch; aborting");
    }
