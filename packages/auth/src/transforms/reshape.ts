import type { BillingMessage } from "../signing/billing.ts";
...
export class ClaudeCodeTransform {
  private readonly catalog: ModelCatalog;
  private readonly billing: BillingHeader;

  constructor(catalog: ModelCatalog, billing: BillingHeader) {
  ...
  apply<T extends ClaudeCodeParams>(params: T): T {
    ...
    const billingHeader = this.billing.build(
      (params.messages ?? []) as BillingMessage[],
      version,
      entrypoint,
    );
    ...
    for (const entry of system) {
      ...
      if (text.startsWith(SYSTEM_IDENTITY) && text.length > SYSTEM_IDENTITY.length) {
        ...
        if (rest) {
          split.push({ ...restProps, type: "text", text: rest });
        }
      } else {
        split.push(entry);
      }
    ...
    if (Array.isArray(params.tools)) {
      params.tools = params.tools.map((t) => ({
        ...t,
        name: t.name ? this.prefixToolName(t.name) : t.name,
      }));
    }
    ...
    if (Array.isArray(params.messages)) {
        ...
        return {
          ...
          content: m.content.map((b) => {
            if (b.type === "tool_use" && typeof b.name === "string") {
              return { ...b, name: this.prefixToolName(b.name) };
            }
      ...
      params.messages = this.repairToolPairs(params.messages);
    }
  ...
  private repairToolPairs(messages: Message[]): Message[] {
    ...
    if (!orphanedUses.size && !orphanedResults.size) {
      return messages;
    }
