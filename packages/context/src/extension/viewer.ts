...
export class Pager implements Component {
  ...
  render(width: number): string[] {
    ...
    for (const line of this.body.split("\n")) {
      ...
      for (const part of wrapTextWithAnsi(line, inner)) {
        wrapped.push(part);
      }
    }
    ...
    if (this.offset > this.lastMax) {
      this.offset = this.lastMax;
    }
    ...
    for (const line of visible) {
      out.push(truncateToWidth(` ${line}`, usable));
    }
...
export class Viewer {
  ...
  async showText(ctx: ExtensionContext, title: string, body: string): Promise<void> {
